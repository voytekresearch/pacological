...
def model(name, time, N_stim, ts_stim, idx_stim, period,
          ...
          w_be, w_bi, r_be, r_bi, Nb=10, N=1000,
          stdp, f=0, seed=None):
    """Model some BRAINS!"""
    ...
    p_back = 0.02
    w_be /= (p_back * Nb) * msiemens
    w_bi /= (p_back * Nb) * msiemens

    # Internal EI connections
    delay = 2 * ms
    p_ei = 0.02
    p_ie = 0.02
    p_e = 0.02
    ...
    w_e /= (p_e * N_e) * msiemens
    w_i = w_i * msiemens
    w_ei /= (p_ei * N_e) * msiemens
    w_ie /= (p_ie * N_i) * msiemens
    w_ee /= (p_e * N_e) * msiemens
    w_ii /= (p_ii * N_i) * msiemens
    ...
    if f > 0:
        f = f * Hz
        P_e_back = NeuronGroup(int(Nb * 0.8), 
                'rates = r_be + (r_be * cos(2 * pi * f * t)) : Hz',
                threshold='rand()<rates*dt')
        P_i_back = NeuronGroup(int(Nb * 0.2), 
                'rates = r_bi + (r_bi * cos(2 * pi * f * t)) : Hz',
                threshold='rand()<rates*dt')
    else:
        P_e_back = PoissonGroup(int(Nb * 0.8), rates=r_be + r_be)
        P_i_back = PoissonGroup(int(Nb * 0.2), rates=r_bi + r_bi)

    C_back_e = Synapses(P_e_back, P_e, pre='g_e += w_be')
    C_back_e.connect(True, p=p_back)
    C_back_i = Synapses(P_i_back, P_e, pre='g_i += w_bi')
    C_back_i.connect(True, p=p_back)

    # --
    # Create network and save
    net = Network(P_e, P_i)
    net.add([P_stim, C_stim_e])
    net.add([C_ee, C_ii, C_ie, C_ei])
    ...
    for k in range(N_trials):
        ...
        traces_e = StateMonitor(P_e, ('V', 'g_e', 'g_i'),
                                record=True)
