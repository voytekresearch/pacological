...
```{r load62, echo=FALSE, warning=FALSE, message=FALSE}
rates <- c(2, 6, 10, 14, 18, 22, 26, 30)
gs <- c(1,2,3,4,5,6,7,8)
Ns <- c(100, 200, 300, 400, 500)
pns <- c(0.25, 0.5, 0.75, 1)

exp62path <- paste(path, "/exp62/", sep="")
mi_exp62 <- load.data(exp62path, "MI", rates, gs, Ns, pns)
h_exp62 <- load.data(exp62path, "H", rates, gs, Ns, pns)
pac_exp62 <- load.data(exp62path, "PAC", rates, gs, Ns, pns)
```

```{r fig62, fig.width=8, fig.height=5, echo=FALSE, warning=FALSE, message=FALSE}
# -- MI
# Sel N and p
mi_fig62 <- filter(mi_exp62, N == 100, pn == 1.00)

# Get Avg MI for stim
Mstim <- mean((filter(mi_fig62, X == "stim_p"))$mean)
mi_fig62$mean <- mi_fig62$mean - Mstim

# Select, rename, and order 'em
mi_fig62 <- filter(mi_fig62, X %in% c("gain_p", "silenced_p", "summed_p"))
mi_fig62$X <- revalue(mi_fig62$X, c("gain_p"="EI", "summed_p"="E", "silenced_p"="I"))
mi_fig62$X <- factor(mi_fig62$X, levels=c("EI", "E", "I")) 

# Sum and plot
mi_stats <- group_by(mi_fig62, X)
mi_stats <- summarise(mi_stats, M=mean(mean), SD=sd(mean))
limits <- aes(ymax = M + SD, ymin=M - SD)

f62a <- qplot(data=mi_stats, x=X, y=M, geom="bar", stat="identity", width=.75) + geom_errorbar(limits, width=0.2, color="grey") + ylab(expression(paste(Delta, "MI"))) + theme_bw() + geom_hline(yintercept=0, color="grey") + xlab("") + ggtitle("Exp 59")

# 2B, Series
mi_exp62 %>% 
  filter(N == 100, pn == 1.00, X %in% c("gain_p", "silenced_p", "summed_p", "stim_p")) %>%
  droplevels() -> mi_fig62b

mi_fig62b$X <- revalue(mi_fig62b$X, c("gain_p"="EI", "summed_p"="E", "silenced_p"="I", "stim_p"="S"))

# Get subtract mean I_stim
mi_fig62b %>% select(mean, Istim, g, X) %>% 
  group_by(Istim, g, X) %>%
...
f62b <- qplot(data=mi_stats, x=Istim, y=value, facets=.~model, color=g, geom="line", group=g) + 
  theme_bw() + 
  xlim(0,30) + 
...
* Under very strong `gmult` E and I PAC have no real effect on MI. Disappointing.
* Try smaller `gmult`.
