...
First we look at the relation between Istim and {MI, H, PAC}, as a function of Iosc magnitude (facet). Metrics for stimulus-only are included for comparison (blue).

```{r, echo=FALSE, fig.width=12, fig.height=10}
p1 <- qplot(data=mi_exp1[mi_exp1$X %in% c("gain_p", "stim_p"),], x=Istim, color=X, y=mean, geom="line", facets=.~Iosc) + theme_bw() + xlim(1,60) + xlab("") + ylab("MI")

p2 <- qplot(data=h_exp1[mi_exp1$X %in% c("gain_p", "stim_p"),], x=Istim, color=X, y=mean, geom="line", facets=.~Iosc) + theme_bw() + xlim(1,60) + xlab("") + ylab("H")

p3 <- qplot(data=pac_exp1[mi_exp1$X %in% c("gain_p", "stim_p"),], x=Istim, color=X, y=mean, geom="line", facets=.~Iosc) + theme_bw() + xlim(1,60) + xlab("Istim") + ylab("PAC")
...
* Effect is quite pronouces for low stimulus firing rates (2 Hz),
* the rapidly declines and recovers, eventually platuaing at or near stimulus MI.
* H increases with gain and Iosc, but the Iosc effect diminshes as Istim and Iosc approach each other.
* Once Iosc hits 10 Hz there is a strong non-linear decline and rise in PAC as a function of Istim. This negative peak shifts leftward as Iosc increases. Curious. Unexpected.
...
p1 <- qplot(data=mi_exp1[mi_exp1$X %in% c("silenced_p", "stim_p"),], x=Istim, color=X, y=mean, geom="line", facets=.~Iosc) + theme_bw() + xlim(1,60) + xlab("") + ylab("MI")

p2 <- qplot(data=h_exp1[mi_exp1$X %in% c("silenced_p", "stim_p"),], x=Istim, color=X, y=mean, geom="line", facets=.~Iosc) + theme_bw() + xlim(1,60) + xlab("") + ylab("H")

p3 <- qplot(data=pac_exp1[mi_exp1$X %in% c("silenced_p", "stim_p"),], x=Istim, color=X, y=mean, geom="line", facets=.~Iosc) + theme_bw() + xlim(1,60) + xlab("Istim") + ylab("PAC")
...
* but in this case increasing Iosc complelly mititgates the effect.

# Binary (synchrony) coding

## Stimulus encoding quality between Poisson and Binary codes.

(An intial quick, dirty, comparison)

First we just compare their respective stimulus encodings. K (network synchrony) was fixed for the binary sample.
...
* By 20 Hz the two codes are equivilant. 
* In all cases Poission coding has higher H.
