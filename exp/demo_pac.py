...
def pa_dist(pha, amp, n_bins=10):
    ...
    return dist, phase_bins

# Run an exp
# Save lfps, binned spikes, make spectrums, and filtered, pac
# Do cross cor between lo and hi and lo/hi
def run(n, t, Iosc, f, Istim, Sstim, dt, k_spikes, excitability,
         pac_type='plv', drivespikes=None, modspikes=None):
    # rate = 1.0 / dt

    # -- SIM ---------------------------------------------------------------------
    # Init spikers
    if modspikes is None:
        modspikes = pac.Spikes(n, t, dt=dt)

    if drivespikes is None:
    ...
    d_bias['stim'] = pac.stim(times, Istim, Sstim, seed=42)
    ...
    for k in d_lfps.keys():
        gamma, theta_phase, d_pacs[k], _ = scpac(d_lfps[k], low_f, high_f, method)
        d_padist[k], bins = pa_dist(theta_phase, gamma, n_bins=21)
...
if __name__ == "__main__":
    ...
    # Setup spikes so all drives have same seed/stim
    drivespikes = pac.Spikes(n, t, dt=dt)
    modspikes = pac.Spikes(n, t, dt=dt)
    ...
    for Iosc, Istim, b_mult in params:
        ...
        res = run(n, t, Iosc, f, Istim, Sstim * Istim, dt, k, excitability,
                  modspikes=modspikes, drivespikes=drivespikes)
        ...
        # PAC dist
        df_dist = pd.DataFrame(res['PACdist'])
        df_dist.to_csv(basepath + "_PACdist.csv")

        # LFPs
        df_lfp = pd.DataFrame(res['lfps'])
        df_lfp.to_csv(basepath + "_lfps.csv")
