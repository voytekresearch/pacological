...
def run(n, t, Iosc, f, Istim, Sstim, dt, k_spikes, excitability,
    ...
    d_pacs = {}
    d_pa = defaultdict(list)
    for k in d_lfps.keys():
        high_power, low_phase, d_pacs[k], _ = scpac(d_lfps[k], low_f, high_f, method)
        for h, l in zip(high_power, low_phase):
            d_pa[np.round(l, decimals=2)].append(h)
