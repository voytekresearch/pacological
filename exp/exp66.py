...
def _run(n, n_b, t, Iosc, f, g, Istim, Sstim, Ipri,
    ...
    # PAC math
    # Unlike exp57 (on which this is based) g is replaced with
    # gmult and fixed at. Meanshile summed and silenced are now in terms
    # of gain, instead of stim. Compare live to commented lines below.
    gmult = 4
    d_bias['gain'] = d_bias['stim'] * (gmult * d_bias['osc'])
    d_bias['summed'] = d_bias['gain'] + (g * d_bias['osc'])
    d_bias['silenced'] = d_bias['gain'] - (g * d_bias['osc'])
