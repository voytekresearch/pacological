...
def _run(n, n_b, t, Iosc, f, g, Istim, Sstim, Ipri,
    ...
    d_spikes['stim_sp'] = stim_sp

    # vars
    d_vars = {}
    for k, v in d_spikes.items():
        d_vars[k] = v.var(1)

    # poprates
    d_poprates = {}
    for k, v in d_spikes.items():
        d_poprates[k] = v.sum(1)
...
def exp(Istim, g, N, pn, n_trials=20):
    ...
    # Var
    # This saves ONLY the last `res` result
    df_lfp = pd.DataFrame(res['vars'])
    df_lfp.to_csv(basepath + "_vars.csv")

    # poprates
    # This saves ONLY the last `res` result
    df_lfp = pd.DataFrame(res['poprates'])
    df_lfp.to_csv(basepath + "_poprates.csv")
