...
if __name__ == "__main__":
    ...
    Js = np.arange(0.0, 3.0, 0.5)
    
    trials = range(n_trials)
    # params = product(trials, Js, Js, Js, Js)
    params = product(trials, Js, Js)
    ...
    Parallel(n_jobs=11)(
        delayed(exp)(name + "_sigma-{0}_Js-".format(sigma) +  # awful name code
            "_".join([str(Je_e), str(Je_i), str(0.0), str(0.0), 
                str(k)]),
            t, dt, Je_e=Je_e, Je_i=Je_i, Ji_e=0.0, Ji_i=0.0, 
            k1=0.5, k2=1.2,
            save=True, no_return=True, 
            seed=k, t_min=1, sigma=sigma
        ) for k, Je_e, Je_i in params
    )      
